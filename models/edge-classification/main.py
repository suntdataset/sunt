...
def run_model(model, 
    ...
    fpath_model =  f'../../outputs/classification/edges/exp_ii/weights/{model_name}.pth'
    ...
    df_loss = pd.DataFrame({'Epochs': np.arange(1, epochs+1),
                            'loss': training_loss})
    fpath_loss =  f'../../outputs/classification/edges/exp_ii/train_loss/loss_{model_name}.csv'
...
def run_gcn(train_dataset, test_dataset, sufix=''):
    ...
    mn = f'gcn{sufix}'
    log.add(f'../../outputs/classification/edges/exp_ii/{mn}.log')
    epcs=50
...
def cross_valition():
    ...
    for fold_idx in np.arange(1, 11):
        ...
        # train_dataset = torch.load(f'../../data/graph_designer/train_test_edge_classification/train_data_{fold_idx}.pt')
        # test_dataset  = torch.load(f'../../data/graph_designer/train_test_edge_classification/test_data_{fold_idx}.pt')
    ...
    fpath_metrcis = '../../outputs/classification/edges/exp_ii/metrics/cross_validation.csv'
